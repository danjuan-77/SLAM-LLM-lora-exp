...
ckpt_path=/mnt/buffer/tuwenming/checkpoints/slam-omni/gpu4-btz1-lr1e-5-warmup_steps5000-SLAM-Omni-fine-tuning-dataset-ultravoice100k_clean-accent/s2s_epoch_3_step_1012/
# jsonl dataset
...
# nohup bash ./examples/s2s/scripts/inference/slam-omni0.5b/inference_s2s_batch_accent.sh > /share/nlp/tuwenming/projects/UltraVoice_dev/logs/run_task_slamomni_inference_gpu0_$(date +%Y%m%d%H%M%S).log 2>&1 &

# nohup bash ./examples/s2s/scripts/inference/slam-omni0.5b/inference_s2s_batch_accent.sh > /share/nlp/kangyipeng/logs/run_task_slamomni_inference_gpu0_$(date +%Y%m%d%H%M%S).log 2>&1 &
